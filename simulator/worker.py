...
def run_simc(run_id: str, profile: str) -> str:
    ...
    # Native armory= import needs our Blizzard credentials; the key comes from
    # env (never stored in the DB profile). Format: "client_id:client_secret".
    key = os.environ.get("BNET_APIKEY", "")
    if key and "\narmory=" in "\n" + profile and "\napikey=" not in "\n" + profile:
